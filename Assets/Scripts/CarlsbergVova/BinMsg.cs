...
// types:
// 00 - null or object type (0)
// 01 - true or bool type (0)
// 02 - false (0)
// 03 - byte (1)
// 04 - char (2)
// 05 - short (2)
...
public class BinMsg {
	...
	UInt32 Adler32(byte[] data, int offset, int length) {
		UInt32 a = 1, b = 0;
		int index;
	...
	byte[] EncodeValue(object o, int estimateSize) {
		...
		//UInt32 crc = Adler32(data, 2 + 4, data.Length);
		// 0 - strings count / 256
		// 1 - objects count / 256
		// 2..5 - adler32 crc
		...
		/*data[2] = (byte)(crc & 255);
		data[3] = (byte)((crc >> 8) & 255);
		data[4] = (byte)((crc >> 16) & 255);
		data[5] = (byte)((crc >> 24) & 255);*/
		return data;
	}
